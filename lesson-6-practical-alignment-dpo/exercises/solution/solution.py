...
#
# DEVICE NOTE: this version runs on CUDA (if present) or CPU only — the Apple
# MPS backend is intentionally not used here. On an Apple-Silicon Mac it will
# run on CPU.
#
...
def _resolve_data_path(filename: str):
    ...
    None. This makes `python solution/solution.py` and `python solution.py`
    both work no matter which folder you launch from.
    """
...
def select_device():
    """Pick the device for training: CUDA if available, otherwise CPU.

    Returns (device_str, dtype). The Apple MPS backend is deliberately excluded
    in this version. CUDA uses float16; CPU uses float32.
    """
...
def run_dpo_training(config: DPOConfig, preference_dataset: Dataset):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        dtype=dtype,
        # device_map="auto" is for multi-GPU/CUDA sharding; for CPU we place
...
if __name__ == "__main__":
    print("DPO Safety Alignment - Fine-Tuning (CPU/CUDA, no MPS)")
    print("=" * 40)
